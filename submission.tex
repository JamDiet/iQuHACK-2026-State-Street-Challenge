...
Beyond the standard comparison, we developed a novel variational approach for \textbf{Conditional Value at Risk (CVaR)} to address a critical limitation in current quantum risk algorithms: the ``Discontinuity Bottleneck.''

\subsection{The Discontinuity Bottleneck}
Standard CVaR estimation relies on the indicator function $\mathbb{1}_{x \ge K}$, which behaves as a Heaviside step function. When approximating this function using polynomial-based techniques (like Quantum Signal Processing), the sharp discontinuity creates the \textit{Gibbs phenomenon}. This requires a polynomial degree that scales as $\mathcal{O}(1/\epsilon)$, leading to deep circuits prone to noise.

\subsection{Resource Analysis: The Algorithmic Gap}
To mitigate this, we applied **Fenchel-Moreau analytic smoothing**, replacing the sharp indicator function with a smooth "Softplus" approximation. This allows us to use **Variational Quantum Signal Processing (V-QSP)** to approximate the risk function with a polynomial degree of $\mathcal{O}(\log(1/\epsilon))$.

We quantified the impact of this smoothing on the total computational cost (Gates/Ops) for a 15-qubit register. As shown in Figure \ref{fig:soft_cvar}, we observe a distinct "Algorithmic Gap" (highlighted in green).

\begin{figure}[H]
    \centering
    \includegraphics[width=0.95\textwidth]{quantum_work_scaling.png}
    \caption{Quantum Work Scaling (15-Qubit Register): Comparison of Classical Monte Carlo (Red), Standard IQAE (Grey), and Soft-CVaR IQAE (Blue). The Soft-CVaR approach significantly lowers the constant factor of the complexity.}
    \label{fig:soft_cvar}
\end{figure}

\paragraph{Key Observations:}
\begin{itemize}
    \item \textbf{Constant Factor Reduction:} While both quantum methods (Grey and Blue lines) exhibit the advantageous Heisenberg scaling (slope $\approx -1$) compared to Classical Monte Carlo (slope $\approx -2$), the Soft-CVaR method is strictly more efficient.
    \item \textbf{Depth Reduction:} For the same target accuracy, Soft-CVaR reduces the circuit depth from $\approx 1500$ (Standard IQAE) to $\approx 318$. This $\sim 5\times$ reduction is critical for execution on NISQ hardware, where coherence time is the limiting resource.
    \item \textbf{Earlier Crossover:} The Soft-CVaR method crosses below the Classical Monte Carlo line at a higher error rate ($\epsilon$) than Standard IQAE. This implies that quantum advantage can be realized with fewer qubits and shallower circuits than previously estimated.
\end{itemize}

\section{Conclusion}
Our analysis demonstrates a clear separation between classical and quantum risk estimation regimes. While classical Monte Carlo remains efficient for rough estimates, IQAE combined with Bisection Search offers a superior scaling pathway. Furthermore, algorithmic innovations like Soft-CVaR prove that by smoothing the mathematical definition of risk, we can reduce circuit depth by factors of $\sim 5\times$, bringing practical quantum advantage significantly closer.
